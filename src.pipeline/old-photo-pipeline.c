...
void* transform1(void* arg) {
    ...
    while (read(pipefd1[0], &params, sizeof(Pipe_params)) > 0) {
        ...
        if (out == NULL) {
            // Handle the case where '/' is not found in the file name
            out = params.file_name;
        }

        fprintf(aux->time, "%s start %jd.%03ld\n", out, start.tv_sec, start.tv_nsec); 
...
void* transform4(void* arg) {
    ...
    while (read(pipefd4[0], &params, sizeof(Pipe_params)) > 0) {
        ...
        if (name_out != NULL) {
            output = (char *)malloc((strlen(params.file_name) + strlen(aux->directory) + 2) * sizeof(char));
            sprintf(output, "%s%s/%s", aux->directory, "/", name_out + 1);
        } else {
            output = (char *)malloc((strlen(params.file_name) + strlen(aux->directory) + 1) * sizeof(char));
            sprintf(output, "%s%s%s", aux->directory, "/", params.file_name);
            name_out = params.file_name; // Set name_out to the full file name in this case
        }
        ...
        fprintf(aux->time, "%s end %jd.%03ld\n", name_out, end_img.tv_sec, end_img.tv_nsec);
        
        free(output); // Free the dynamically allocated memory for output
    }
}
...
int main(int argc, char *argv[]){
	...
	char timing[256];
	sprintf(timing, "%s%s", argv[1], "/timing_pipeline.txt");
	...
	thread_res.time = timing_n;
	thread_in.time = timing_n;
